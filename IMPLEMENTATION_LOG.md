...
**Rust Backend:**
- `src-tauri/src/lib.rs` — registered global shortcut plugin; set up Cmd+Option+. shortcut in `setup()`; emits `hotkey-triggered` event on press; added `chrono_now_iso()` and `civil_from_days()` helpers for timestamp generation; imported `tauri::Emitter` and `tauri::Manager` traits
...
- `src/lib/hotkeys.ts` — NEW: default hotkey config (`Cmd+Option+.`), `getDefaultHotkeyLabel()`, `normalizeShortcutLabel()`, `formatLastTriggered()`, `formatTriggerCount()` helpers
...
3. **Hotkey → real recording**: Cmd+Option+. now toggles actual microphone recording (previously toggled placeholder state only).
